...
def find_bubble_location_in_grid(bullet_bubble):
    ...
    rows_index_list = []
    for row_i in range(len(bubbles_grid)):  # find the last row index
        rows_index_list.append(row_i)  # make a list of all the indexes
    last_row_index = rows_index_list[-1]  # access the last index
    for row in range(len(bubbles_grid)):
        if row == last_row_index:
            bubble_list = []
            distance_list = []
            for col in range(len(bubbles_grid[row])):  # check every empty bubble
                bubble = bubbles_grid[row][col]  # bubble will be a dict
                b_center_x = bubble["center_x"]
                b_center_y = bubble["center_y"]
                bubble_cord = [b_center_x, b_center_y]
                distance = math.dist(bullet_bubble_cord, bubble_cord)
                # check if overlapping - there can be two suitable bubbles and we want the min distance
                if distance < consts.BUBBLE_RADIUS * 2:
                    bubble_list.append((row, col))
                    distance_list.append(distance)
            if len(distance_list) > 1:
                min_dist = min(distance_list, key=lambda x: float(x))
                index = distance_list.index(min_dist)
                res = bubble_list[index]
                return res
...
def colors_in_grid():
    ...
    for row in range(len(bubbles_grid)):
        for col in range(len(bubbles_grid[row])):
            ...
            bubble_color = bubble["color"]  # also the color here arrives as a tuple
            colors_list_grid.append(bubble_color)
        return colors_list_grid
